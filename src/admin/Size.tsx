...
import { API_SIZE_URL, API_SEARCH_SIZE_URL } from "../constants";
...
const Sizes = () => {
  ...
  const [editableSize, setEditableSize] = useState<{
    id: number;
    name: string;
  } | null>(null);
  useEffect(() => {
    const fetchedsizes = async () => {
      setLoading(true);
      try {
        const response = await authenticatedAxios.get(API_SIZE_URL);
    ...
    fetchedsizes(); // Don't forget to call the function to trigger the useEffect
  }, []); // Add an empty dependency array to run the effect only once on mount

  const handleEditClick = (Size: Size) => {
    setEditableSize({ id: Size.id, name: Size.name });
  };
  ...
  const handleSaveClick = async () => {
    if (editableSize) {
      try {
        setLoading(true);
        // Make a request to update the Size name
        await authenticatedAxios.put(`${API_SIZE_URL}${editableSize.id}/`, {
          name: editableSize.name,
        });
        // Update the sizes state after successful update
        setsizes(
          (prevsizes) =>
            prevsizes &&
            prevsizes.map((Size) =>
              Size.id === editableSize.id
                ? { ...Size, name: editableSize.name }
                : Size
            )
        );

        setEditableSize(null); // Disable editing mode
      } catch (error) {
        // handle error
      } finally {
        setLoading(false);
  ...
  const addNewSize = async () => {
    if (newSize) {
      try {
        setLoading(true);
        // Make a request to update the Size name
        const response = await authenticatedAxios.post(`${API_SIZE_URL}`, {
          name: newSize,
        });
  ...
  const handleCancelClick = () => {
    setEditableSize(null); // Cancel editing
  };

  const handleNameChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (editableSize) {
      setEditableSize((prevEditableSize) => ({
        ...prevEditableSize!,
  ...
  const handleSearch = async (searchKeyword: string) => {
    try {
      ...
      const response = await authenticatedAxios.get(
        `${API_SEARCH_SIZE_URL}?search=${searchKeyword}`
      );
